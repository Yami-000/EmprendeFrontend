...
export default function Header({ onMenu, onProfile, onLogout, profile, user }) {
  const label = user?.displayName || user?.email || profile.name
    ...
    return (
        <header className="w-full h-14 flex items-center justify-between px-4 border-b border-white/6 bg-[#0d1f17] text-[#e9f1ff] backdrop-blur-md">
            <div className="flex items-center">
                <button className="text-white text-xl bg-transparent" onClick={onMenu} aria-label="Abrir menú">☰</button>
            </div>

            <div className="flex-1 flex justify-center">
                <h1 className="text-white text-lg font-semibold tracking-wide">Emprende</h1>
            </div>
            ...
            <div className="flex items-center">
                ...
                <button className="bg-transparent mr-3" onClick={onProfile} aria-label="Perfil">
                    <span className="w-9 h-9 rounded-full bg-gradient-to-br from-[#0CD138] to-[#06a321] flex items-center justify-center font-semibold text-sm text-white shadow">{label.charAt(0).toUpperCase()}</span>
                </button>
